...
class DriverTripController extends Controller
{
    ...
    public function startTrip(Request $request)
    {
        ...
        $startTrip = Trip::where('trip_id', $request->tripId)->update(['trip_status' => 2, 'trip_start_kilometer' => $request->tripStartKm, 'trip_start_datetime' => date('Y-m-d H:m:s')]);
        if ($startTrip == 1) {
    ...
    {
        Trip::findOrFail($request->tripId);
        $startTrip = Trip::where('trip_id', $request->tripId)->update(['trip_status' => 3, 'trip_end_kilometer' => $request->tripEndkm,  'trip_end_datetime' => date('Y-m-d H:m:s')]);
        if ($startTrip == 1) {
            return response(['result' => true]);
        } else {
            return response(['result' => false]);
